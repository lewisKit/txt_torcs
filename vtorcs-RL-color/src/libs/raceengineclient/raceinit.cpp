...
/* Sort race managers by priority sorted from small to large*/
static void
...
void ReAddRacemanListButton(void *menuHandle)
{
	...
	do
	{
		...
		//printf("looping dispName %s , and name %s , get LocalDir %s \n", racemanCur->dispName, racemanCur->name, GetLocalDir());
		if (getTextOnly()==false)
		{
...
//				printf("Starting Quick Race \n");
		...
		{
			if(strcmp(racemanCur->dispName, "Practice") == 0)
			{
				printf("Starting Practice \n");
				const char* raceconfig = "";
				raceconfig = GetRaceConfig();
				// reconfig parameter files using a custom one only in text mode
				char bufd[1024];
				sprintf(bufd, "%s", GetRaceConfig());
				...
				if(strlen(raceconfig) == 0) {
					...
					racemanCur->userData = GfParmReadFile(bufd, GFPARM_RMODE_STD);
					if (racemanCur->userData == 0) {
...
ReInitTrack(void)
{
	...
	if (strlen(GetTrackInfo()) == 0){
		...
		std::vector<char*> vv;
		char* chars_array = strtok(GetTrackInfo(), "*");
		while(chars_array)
		{
			vv.push_back(chars_array);
			chars_array = strtok(NULL, "*");
		}
		printf("trackName, %s; catname, %s\n", vv[0], vv[1]);
		trackName = strdup(vv[0]);
		if (!trackName) return -1;
		catName = strdup(vv[1]);
		vv = std::vector<char*>();
		free(chars_array);
