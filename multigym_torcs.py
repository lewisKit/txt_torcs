...
class TorcsEnv:
    ...
    def start_torcs_process(self):
        ...
        torcs_cmd = os.path.join(os.path.expanduser('~'), 'game/bin/torcs')
        if self.text_mode:
    ...
    def step(self, u):
       #print("Step")
        ...
        progress = sp*(np.cos(obs['angle']) - np.abs(np.sin(obs['angle'])) - np.abs(obs['trackPos'])) / self.default_speed
        reward = progress

        # collision detection
        if obs['damage'] - obs_pre['damage'] > 0:
            reward = -200 / self.default_speed
